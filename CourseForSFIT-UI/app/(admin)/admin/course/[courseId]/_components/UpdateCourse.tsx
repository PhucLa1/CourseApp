...
import { GetAllCourseType, GetCourseById, UpdateCourseById } from '@/apis/course.api'
import {
    Select,
    SelectContent,
    SelectGroup,
...
export default function UpdateCourse({courseId} : {courseId: number}) {
    ...
    const [learnAbout, setLearnAbout] = useState<boolean[]>([])
    ...
    const {data: dataCourseDetail, isLoading: isLoadingCourseDetail} = useQuery({
        queryKey: ['courseDetail'],
        queryFn: () => GetCourseById(courseId)
    })
    const { mutate, isPending } = useMutation({
        mutationFn: (data: FormData) => {
            return UpdateCourseById(courseId,data)
        },
        onSuccess(data, variables, context) {
            if(data.data.isSuccess){
                toast.success("Thành công sửa thông tin khóa học")
    ...
    const OnUpdateCourse = () => {
        ...
        let listLearnAboutIndex = 0
        let listPreparedIndex = 0
        listLearnAbout.forEach((item, index) => {
            if(item != null && item != "") {formData.append(`listLearnAbout[${listLearnAboutIndex}]`, item.toString()); listLearnAboutIndex++    }       
        });
        listPrepared.forEach((item, index) => {
            if(item != null && item != "") {formData.append(`listPrepared[${listPreparedIndex}]`, item.toString()); listPreparedIndex++}
        });
    ...
    useEffect(() => {
        if(dataCourseDetail){
            const data = dataCourseDetail.data.metadata
            setCourseType(data.courseTypeId.toString())
            setStatus(data.status.toString())
            setCoureName(data.name)
            setDescription(data.description)
            setListLearnAbout(data.listLearnAbout ?? [])
            setListPrepared(data.listPrepared ?? [])
            setPrepared((data.listPrepared ?? []).map(() => false));
            setLearnAbout((data.listLearnAbout ?? []).map(() => false));
            if(data.thumbnail != null) setImagePreviewUrl(`https://localhost:7130/Uploads/${data.thumbnail}`)
        }
    },[dataCourseDetail])
    return (
        <div>
            ...
            <div className='flex items-center justify-start'>
                <Button onClick={() => OnUpdateCourse()} variant='default'>Sửa khóa học</Button>
